...
Neither of the distributions seems normal, the first one tends toward normality even though it's a bit left skewed, but the second one is very clearly right-skewed.
...
Yes, the savings ratio seems to be closer to a normal distribution, as the points are closer to the line, while the disposable income is clearly not normal, as the points deviate significantly from the line, especially at the tails.
...
For the savings ratio, the p-value is greater than 0.05, so we fail to reject the null hypothesis and can treat the data as approximately normal. The W statistic is also relatively close to 1, supporting this conclusion.

For the disposable income, the p-value is less than 0.05, so we reject the null hypothesis and conclude that the data are not normally distributed. The W statistic is still close to 1, but the low p-value indicates a significant deviation from normality.
...
No, neither transformation makes it normally distributed, both p-values are below 0.05 (we can reject the null hypothesis)
...
It is not easy to directly interpret, because the values are in terms of standard deviations from the mean, rather than in the original units of measurement. This can make it harder to understand what a specific value means in a real-world context. But it makes it easier to compare across variables.
...
A Pearson correlation coefficient was computed to assess the linear relationship between savings ratio and disposable income. There was no statistically significant correlation between the two variables, r(48) = .22, p = .13, the CI was [.06, .47].
...
Spearman’s rank correlation was computed to assess the relationship between savings ratio and disposable income. There was a positive correlation between the two variables, r(48) = .28, p = 0.04.
...
The two tests have different underlying assumptions and are sensitive to different aspects of the data. Pearson's correlation assumes normality and linearity, while Spearman's correlation is non-parametric and based on ranks. In this case, the data for disposable income is not normally distributed, which violates the assumptions of Pearson's test, leading to a non-significant result. Spearman's test, being more robust to non-normality and outliers, detected a significant relationship between the variables.
...
**Question of interest**: Is there a correlation between life expectancy and illiteracy?
...
From these measures we can conclude that life expectancy variable is approximately normally distributed, while the illiteracy variable is not normally distributed.
...
This means log transformation makes the illiteracy variable approximately normally distributed (although based on the histogram it is still very skewed), while the square root transformation does not. I will continue the analysis with the log-transformed illiteracy variable.
...
A pearson correlation was computed to assess the linear relationship between life expectancy and log-transformed illiteracy. There was a statistically significant negative correlation between the two variables, r(48) = -.57, p < .001, the CI was [-.73, -.34].

The spearmans rank correlation was also conducted to assess robustness of results. There was a negative correlation between the two variables, r(48) = -.55, p < .001.
